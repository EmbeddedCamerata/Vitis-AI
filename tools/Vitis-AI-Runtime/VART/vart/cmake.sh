...
options=$(getopt -a -n 'parse-options' -o h \
		 -l help,clean,conda,build-only,build-python,type:,pack:,build-dir:,install-prefix:,cmake-options: \
		 -- "$0" "$@")
[ $? -eq 0 ] || {
...
# set build dir
build_dir_default=../build/build.${target_info}/${project_name}
...
if [ ${show_help:=false} == true ]; then
    ...
    echo "    --build-python            build python"
